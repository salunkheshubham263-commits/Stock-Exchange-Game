...
from sqlcipher3 import dbapi2 as sqlite3  # Use SQLCipher
...
def fetch_stock_price(symbols):
    now = time.time()
    if symbols in price_cache and now - last_fetch_time.get(symbols, 0) < 5:
...
def buy_stock():
    ...
    # Update holdings
    holding = cur.execute(
        "SELECT quantity FROM Holdings WHERE user_id=? AND symbol=?", (user_id, symbol)
    ).fetchone()
    ...
    return jsonify({
        "status": "ok",
        "message": f"Bought {qty} shares of {symbol}",
